...
// Normal solution is that when we use a two arrays to store the paths of the nodes while traversing them two 
// times (one time for each) -- TC : O(N) + O(N) , SC : O(N) + O(N)
