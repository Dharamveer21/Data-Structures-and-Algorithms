...
public:
    ...
    {
        vector<vector<vector<int>>> max_profit(prices.size(), vector<vector<int>>(2, vector<int>(k + 1, -1)));
        return maximum_profit(0, 0, k, prices, max_profit);
    }
    ...
    {
        int n = prices.size();
        int max_profit[prices.size()][2][k + 1];

        for (int day = n - 1; day >= 0; day--)
        {
            for (int has_stock = 1; has_stock >= 0; has_stock--)
            {
                for (int tran_cnt = 0; tran_cnt < k + 1; tran_cnt++)
                {
                    if (has_stock)
                    {
    ...
    {
        int n = prices.size();
        int curr_max_profit[2][k + 1];
        int next_max_profit[2][k + 1];
