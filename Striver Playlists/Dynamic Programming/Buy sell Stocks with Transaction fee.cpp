...
public:
    // memoisation
    int max_profits(int day, int has_stock, int &fee, vector<int> &prices, vector<vector<int>> &maxi_profit)
    {
        if (day == prices.size())
        {
            if (has_stock)
        ...
        {
            // sell it
            int sell_profit = prices[day] - fee + max_profits(day + 1, 0, fee, prices, maxi_profit);

            // don't sell it
            int not_sell_profit = max_profits(day + 1, 1, fee, prices, maxi_profit);
        ...
        // buy it
        int buy_profit = (-1) * prices[day] + max_profits(day + 1, 1, fee, prices, maxi_profit);

        // don't buy it
        int not_buy_profit = max_profits(day + 1, 0, fee, prices, maxi_profit);
    ...
    int maxProfit(vector<int> &prices, int fee)
    {
        ...
        return max_profits(0, 0, fee, prices, maxi_profit);
    }
    ...
    int maxProfit(vector<int> &prices, int fee)
    {
        ...
        for (int day = n - 1; day >= 0; day--)
        {
            for (int has_stock = 0; has_stock < 2; has_stock++)
            {
                ...
                {
                    // sell it
                    int sell_profit = (day == n - 1) ? prices[day] - fee : prices[day] - fee + maxi_profit[day + 1][0];
    ...
    int maxProfit(vector<int> &prices, int fee)
    {
        ...
        for (int day = n - 1; day >= 0; day--)
        {
            for (int has_stock = 0; has_stock < 2; has_stock++)
            {
                ...
                {
                    // sell it
                    int sell_profit = (day == n - 1) ? prices[day] - fee : prices[day] - fee + next_maxi_profit[0];
